...
class SDXLInpainter:
    """
    SDXL inpainting adapter.

    Best for:
        - larger watermark regions
        - complex backgrounds
    ...
    def __init__(
        self,
        config: SDXLInpainterConfig | None = None,
        pipe: Any | None = None,
    ) -> None:
        self.config = config or SDXLInpainterConfig()
        self.pipe = pipe
    ...
    def _load_pipeline(self) -> Any:
        ...
        pipeline_cls = getattr(diffusers, "StableDiffusionXLInpaintPipeline")
...
def sdxl_inpaint(
    ...
    config = SDXLInpainterConfig(device=device)

    if prompt is not None:
        config.prompt = prompt

    inpainter = SDXLInpainter(config)
